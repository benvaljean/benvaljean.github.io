...
<!-- -->
    ...
    define service{
            use                             service
            host_name                       dbserver
            service_description             MySQL load check
...
<!-- -->
    ...
    define command{
            ...
            command_line            $USER1$/check_mysql_health --host 127.0.0.1 -P 3335 --mode $ARG1$ -u nagios -p Rogerrabbit45 -w $ARG2$ -c $ARG3$ --database aspire_production
    }

    define command{
            command_name            check_mysql_health_db02
            command_line            $USER1$/check_mysql_health --host 127.0.0.1 -P 3336 --mode $ARG1$ -u nagios -p ohS0aiPu -w $ARG2$ -c $ARG3$ --database aspire_production
    }
...
specific WMI syntax. Details on how to monitor the following are
explanied:

-   SQL Services (check\_nt)
-   Ports open (check\_tcp)
-   Load / health monitoring (check\_nt and WMI counters)
-   SQL Mirroring (check\_nt and WMI counters)
...
[Categroy:Nagios](Categroy:Nagios "wikilink")
