...
3.  make
4.  sudo make install
5.  Edit your /usr/local/etc/freetds.conf to include your SQL server
    hostname and port. This is important as a \"-S\" parameter refers to
    the configuration entry here and not the hostname itself.
6.  Test the config:
...
the plugin can be used in the following service and command config:
    ...
    define command{
            check_command           check_mssql
            command_line            $USER1$/check_mssql.sh $HOSTADDRESS$ -U nagios -P Rogerrabbit45!
    }
